...
WCHAR		g_szAppTitle[]	= L"Direct3D 11 Sample11";
WCHAR		g_szWndClass[]	= L"D3D11S11";
...
LRESULT CALLBACK MainWndProc(HWND hWnd, UINT msg, UINT wParam, LONG lParam)
{
	...
	case WM_KEYDOWN:
		...
		case VK_F3:		// [F3]キーでフラクタル図形のテクスチャを保存
			if (g_pTexture != NULL) {
				D3DX11SaveTextureToFile(g_pImmediateContext,// デバイス・コンテキスト
										g_pTexture,			// 保存するテクスチャ
										D3DX11_IFF_BMP,		// Windowsビットマップ形式で保存
										L"save_tex.bmp");   // 保存先のファイル名
			}
			break;

		case VK_F4:		// [F4]キーでバックバッファを保存
			if (g_pRenderTargetView != NULL) {
				ID3D11Resource* pBackBuffer;
				g_pRenderTargetView->GetResource(&pBackBuffer);
				D3DX11SaveTextureToFile(g_pImmediateContext,// デバイス・コンテキスト
										pBackBuffer,		// 保存するバック バッファ
										D3DX11_IFF_BMP,		// Windowsビットマップ形式で保存
										L"save_back.bmp");  // 保存先のファイル名
