...
WCHAR		g_szAppTitle[]	= L"Direct3D 11 Sample08";
WCHAR		g_szWndClass[]	= L"D3D11S08";
...
HRESULT InitDirect3D(void)
{
	...
	D3DX11CreateTextureFromFile(
				...
				L"..\\misc\\texture2.png", // 画像ファイル名
				&imageLoadInfo,  // 作成するテクスチャの設定
				NULL,            // 非同期で実行しない
				&g_pTexture,     // テクスチャを取得する変数
				&hr);            // 戻り値
	if (FAILED(hr))
		return DXTRACE_ERR(L"InitDirect3D D3DX11CreateTextureFromFile", hr);
