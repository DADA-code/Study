...
WCHAR		g_szAppTitle[]	= L"Direct3D 11 Sample09";
WCHAR		g_szWndClass[]	= L"D3D11S09";
...
ID3D11Texture2D*          g_pRTTexture = NULL;		// 描画可能テクスチャ
ID3D11RenderTargetView*   g_pRTTextureRTV = NULL;	// 描画ターゲット ビュー
ID3D11ShaderResourceView* g_pRTTextureSRV = NULL;	// シェーダ リソース ビュー
D3D11_VIEWPORT            g_vpRTTexture[1];			// 描画可能テクスチャのビューポート設定
ID3D11Texture2D*          g_pRTDepthStencil = NULL;		// 深度/ステンシル
ID3D11DepthStencilView*   g_pRTDepthStencilView = NULL;	// 深度/ステンシル・ビュー
...
// 描画ターゲットをクリアする値(RGBA)
float g_ClearColor[2][4] = {{ 0.0f, 0.125f, 0.3f, 1.0f }, { 0.3f, 0.125f, 0.0f, 1.0f }};
...
HRESULT InitDirect3D(void)
{
	...
	// **********************************************************
	// シェーダ リソース ビューの作成
	D3DX11CreateShaderResourceViewFromFile(g_pD3DDevice, L"..\\misc\\texture2.png", NULL, NULL, &g_pTextureSRV, &hr);
	if (FAILED(hr))
		return DXTRACE_ERR(L"InitDirect3D D3DX11CreateShaderResourceViewFromFile", hr);
	...
	hr = g_pD3DDevice->CreateTexture2D(&desc, NULL, &g_pRTTexture);
	if (FAILED(hr))
	...
	hr = g_pD3DDevice->CreateRenderTargetView(
			g_pRTTexture,      // ビューを作るテクスチャ リソース
			&rtDesc,           // ビューの設定
			&g_pRTTextureRTV); // ビューを受け取る変数
	if (FAILED(hr))
		return DXTRACE_ERR(L"InitDirect3D g_pD3DDevice->CreateRenderTargetView", hr);
	...
	hr = g_pD3DDevice->CreateShaderResourceView(
			g_pRTTexture,       // ビューを作るテクスチャ リソース
			&srDesc,            // ビューの設定
			&g_pRTTextureSRV);  // ビューを受け取る変数
	if (FAILED(hr))
		return DXTRACE_ERR(L"InitDirect3D g_pD3DDevice->CreateShaderResourceView", hr);

	// ビューポートの設定
	g_vpRTTexture[0].TopLeftX = 0.0f;		// ビューポート領域の左上X座標。
	g_vpRTTexture[0].TopLeftY = 0.0f;		// ビューポート領域の左上Y座標。
	g_vpRTTexture[0].Width    = (FLOAT)desc.Width;	// ビューポート領域の幅
	g_vpRTTexture[0].Height   = (FLOAT)desc.Height;// ビューポート領域の高さ
	g_vpRTTexture[0].MinDepth = 0.0f;	// ビューポート領域の深度値の最小値
	g_vpRTTexture[0].MaxDepth = 1.0f;	// ビューポート領域の深度値の最大値

	// 描画ターゲット用の深度/ステンシル テクスチャの作成
	D3D11_TEXTURE2D_DESC descDepth;
	descDepth.Width     = 256;   // 幅
	descDepth.Height    = 256;  // 高さ
	...
	hr = g_pD3DDevice->CreateTexture2D(
			...
			NULL,					// 
			&g_pRTDepthStencil);	// 作成したテクスチャを受け取る変数
	if (FAILED(hr))
		return DXTRACE_ERR(L"InitDirect3D g_pD3DDevice->CreateTexture2D", hr);  // 失敗
	...
	hr = g_pD3DDevice->CreateDepthStencilView(
			g_pRTDepthStencil,		// 深度/ステンシル・ビューを作るテクスチャ
			&descDSV,				// 深度/ステンシル・ビューの設定
			&g_pRTDepthStencilView);// 作成したビューを受け取る変数
	if (FAILED(hr))
		return DXTRACE_ERR(L"InitDirect3D g_pD3DDevice->CreateDepthStencilView", hr);  // 失敗
...
HRESULT Render(void)
{
	...
	g_pImmediateContext->ClearRenderTargetView(
						g_pRTTextureRTV,	// クリアする描画ターゲット
						g_ClearColor[1]);	// クリアする値

	// 深度/ステンシルのクリア
	g_pImmediateContext->ClearDepthStencilView(
			g_pRTDepthStencilView,	// クリアする深度/ステンシル・ビュー
			D3D11_CLEAR_DEPTH,		// 深度値だけをクリアする
			1.0f,					// 深度バッファをクリアする値
			0);						// ステンシル・バッファをクリアする値(この場合、無関係)

	// RSにビューポートを設定
	g_pImmediateContext->RSSetViewports(1, g_vpRTTexture);

	// PSにシェーダ リソース ビューを設定する
	g_pImmediateContext->PSSetShaderResources(0, 1, &g_pTextureSRV);
	// PSにサンプラーを設定
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pTextureSampler);

	// OMに描画ターゲット ビューを設定
	g_pImmediateContext->OMSetRenderTargets(1, &g_pRTTextureRTV, g_bDepthMode ? g_pRTDepthStencilView : NULL);
	...
	g_pImmediateContext->PSSetShaderResources(
        ...
        &g_pRTTextureSRV);	// 設定するシェーダ・リソース・ビューの配列
	// PSにサンプラーを設定
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pTextureSampler);
...
bool CleanupDirect3D(void)
{
	...
	// 取得したインターフェイスの開放
	SAFE_RELEASE(g_pRTDepthStencilView);
	SAFE_RELEASE(g_pRTDepthStencil);
	SAFE_RELEASE(g_pRTTextureRTV);
	SAFE_RELEASE(g_pRTTextureSRV);
