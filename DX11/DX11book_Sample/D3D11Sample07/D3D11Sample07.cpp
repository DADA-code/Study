...
WCHAR		g_szAppTitle[]	= L"Direct3D 11 Sample07";
WCHAR		g_szWndClass[]	= L"D3D11S07";
...
HRESULT InitDirect3D(void)
{
	...
	// リソースの種類
	ID3D11Resource* pResource = NULL;   // テクスチャ リソースを受け取る変数
	g_pTextureSRV->GetResource(&pResource);

	D3D11_RESOURCE_DIMENSION type;	// リソースの種類を受け取る変数
	...
	switch (type)
	{
		case D3D11_RESOURCE_DIMENSION_TEXTURE1D:
		{
			...
			WCHAR buffer[128];
			swprintf_s(buffer, 128, L"1DTexture Width=%i", desc.Width);
			DXTRACE_MSG(buffer);
			break;
		}
		...
		case D3D11_RESOURCE_DIMENSION_TEXTURE2D:
		{
			...
			WCHAR buffer[128];
			swprintf_s(buffer, 128, L"2DTexture Width=%i Height=%i", desc.Width, desc.Height);
			DXTRACE_MSG(buffer);
			break;
		}
		...
		case D3D11_RESOURCE_DIMENSION_TEXTURE3D:
		{
			...
			WCHAR buffer[128];
			swprintf_s(buffer, 128, L"3DTexture Width=%i Height=%i Depth=%i", desc.Width, desc.Height, desc.Depth);
			DXTRACE_MSG(buffer);
			break;
		}
		...
		{
			// エラー(テクスチャ リソースではない)
			DXTRACE_MSG(L"UNKNOWN or BUFFER Resource");
			break;
		}
	}
